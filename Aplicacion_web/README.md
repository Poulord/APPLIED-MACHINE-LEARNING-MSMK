...
Activa la cámara del navegador, usa **OpenCV.js** para detectar múltiples objetos generales a partir
de bordes/contornos (sin depender de colores específicos), muestra etiqueta y confianza, y envía un
log anónimo a una función serverless en Vercel.

## Características
- **Vista previa de cámara** usando `navigator.mediaDevices.getUserMedia`.
- **Detección multi-objeto con OpenCV.js** basada en bordes (Canny), dilatación y contornos para
  extraer bounding boxes, etiqueta y confianza (al estilo Ultralytics: clase + score).
...
- `script.js`: lógica de cliente (estado centralizado, OpenCV.js para detección por bordes/contornos, overlay, envío de log).
...
3. **Detección en vivo**: cada frame procesado se pasa a escala de grises, se suaviza, se aplica Canny + dilatación y se extraen contornos. Cada contorno válido genera una detección con bounding box y confianza derivada del área relativa.
...
```json
{
  ...
  "detections": [
    { "label": "Objeto 01", "confidence": 0.82, "box": { "x": 10, "y": 20, "width": 180, "height": 120 } },
    { "label": "Objeto 02", "confidence": 0.65, "box": { "x": 220, "y": 60, "width": 120, "height": 200 } }
  ],
