...
Activa la cámara del navegador, usa **OpenCV.js** para detectar múltiples objetos según su color
(rojo, verde y azul) mostrando etiqueta y confianza, y envía un log anónimo a una función
serverless en Vercel.

## Características
- **Vista previa de cámara** usando `navigator.mediaDevices.getUserMedia`.
- **Detección multi-objeto con OpenCV.js** basada en segmentación de color (HSV) y contornos,
  generando bounding boxes, etiqueta y confianza (al estilo Ultralytics: clase + score).
...
- `script.js`: lógica de cliente (estado centralizado, OpenCV.js para detección por color, overlay, envío de log).
...
2. **Cámara**: se solicita permiso y se inicializa el stream. El `<video>` y `<canvas>` se sincronizan en tamaño.
3. **Detección en vivo**: cada frame procesado convierte a HSV, aplica máscaras para colores primarios y extrae contornos. Cada contorno genera una detección con bounding box y confianza derivada del área relativa.
...
```json
{
  ...
  "detections": [
    { "label": "objeto rojo", "confidence": 0.82, "box": { "x": 10, "y": 20, "width": 180, "height": 120 } },
    { "label": "objeto verde", "confidence": 0.65, "box": { "x": 220, "y": 60, "width": 120, "height": 200 } }
  ],
...
- Ajusta los rangos HSV en `COLOR_RANGES` dentro de `script.js` para detectar otros colores u objetos.
